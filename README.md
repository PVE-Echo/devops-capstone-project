...
このリポジトリは、DevOpsの学習・実践の総仕上げ（キャップストーンプロジェクト）として作成されたものです。
CI/CDパイプラインの構築、コンテナ化、自動テスト、クラウドインフラストラクチャの管理などの実践的な演習を行います。

![Build Status](https://github.com/PVE-Echo/devops-capstone-project/actions/workflows/ci-build.yaml/badge.svg)
```
